...
def gen_usage(iteration: str, in_str_dict: dict):
    ...
    result += (
        ...
        f"*[http://kenyu1234.php.xdomain.jp/resultsm.php?sen=0&pd={kenyu_int}&mn=8 "
        f"第{iteration_int}期棋聖戦：将棋棋士成績DB]\n"
        f"*[http://shogititle.nobody.jp/table/kisei/kisei-{str(iteration_int).zfill(2)}.html "
        f"第{iteration_int}期棋聖戦：将棋タイトル戦]\n"
        "{{各期の棋聖戦}}\n"
        "{{Shogi-stub}}\n"
