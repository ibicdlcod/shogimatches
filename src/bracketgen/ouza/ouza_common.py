...
def ouza_title_matches(iteration_int: int):
    ...
    # 五番勝負
    title_matches = sql_read.read_match("王座戦", iteration_str, "タイトル戦", "五番勝負")
    if iteration_int == 31:
        ...
        title_matches_last = sql_read.read_match("王座戦", iteration_str_prev, "タイトル戦", "五番勝負")
        org_tree_title_last = organized_tr.OrganizedTree(title_matches_last, f"タイトル戦五番勝負", ["", ])
    org_tree_title = organized_tr.OrganizedTree(title_matches, f"タイトル戦五番勝負", ["", ])
...
def ouza_pseudo_title_matches(iteration_int: int):
    ...
    if iteration_int <= 18:
        ...
        title_matches_last = sql_read.read_match("王座戦", iteration_str_prev, "準タイトル戦", "三番勝負")
        org_tree_title_last = organized_tr.OrganizedTree(title_matches_last, f"準イトル戦三番勝負", ["", ])
